...
\TODO{explain the three phases: analyse, factorization and solve} The
supernodal Cholesky method is a factorization algorithm for sparse
matrices where the input matrix $A$ is decomposed as:
\begin{equation}\label{eq:chol}
  A = LL^{T}
\end{equation}
where the factor $L$ is a lower triangular matrix \TODO{quite word on
  the solve phase (backward and forward substitutions, multiple rhs,
  etc)}. Note that the sparsity pattern of the factor is not the same
as the original matrix as because additional coefficients generally
appear during the computation of this factor resulting in a denser
matrix than the original one. This phenomenon is called
...
we transform the original matrix into $PAP^{T}$ \TODO{AMD
  vs. nested-dissection ordering}. The dependencies between the
...
separately \alert{give more explanation about this
  strategy. Separately is misleading. talk about the distribution of
  these two levels of parallelism} . Instead, in our work, we follows
the approach proposed in~\cite{h.r.s:10} where supernodes are
...
right-looking scheme. \TODO{remarks on the different schemes: fan-in,
  fan-out, etc}
...
developed another version based on the StarPU runtime system. As shown
in the next section, both implementations of our solver rely on a STF
model, but the StarPU-based implementation can benefit from a wider
range of features available with StarPU. For example, although we
focus on shared-memory architectures in this work, the StarPU version
can be extended to a distributed-memory version whereas OpenMP doesn't
provide any possibility for execution on distributed memory
architectures.
...
\TODO{detail example code for OpenMP.}
...
\TODO{explain dynamic scheduling of ready tasks.}
...
\TODO{In a new section, present MA87 and especially PTG-like
  programming model.}
