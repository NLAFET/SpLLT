...
ordering \metis. As explained in Section~\ref{sec:chol} this allows us
to reduce the number of entries in the factor $L$ by limiting the
fill-in. Note that in this table, matrix characteristics are obtained
without node amalgamation. This means that the number of entries in
$L$ as well as the flop count is minimized but in our experiment we
exploit node amalgamation to have a better efficiency of operations at
the cost of a larger number of entries in $L$ as well as an increase
in the floating point operation count.\alert{is the last sentence
  necessary?}.
