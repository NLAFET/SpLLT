...
performance and scalability. They first have been widely employed in
the context dense linear algebra such as in the
PLASMA~\cite{a.d.d.h.ea:09} software package. Motivated by the extreme
efficiency provided by these algorithms, they have been adapted to
sparse algorithms. For example, The \texttt{HSL\_MA87}
solver~\cite{h.r.s:10} implements a DAG-based sparse Cholesky
factorization and the \texttt{qr\_mumps} solver~\cite{b:13} implements
a DAG-based multifrontal QR method.

Classically the development of task-based algorithms include the
development of an ad-hoc scheduler which relies on the knowledge of
the algorithm and implemented using a low-level multithreading library
such as pthread (POSIX threads) to manage synchronisations and enforce
dependencies between processes. In this work, instead, we explore
another approach based on the use of a runtime system which can be
...
a high-level API provided by the runtime system the low level details
such as data consistency across the architecture and task scheduling
are delegated to the runtime system. On top of this, we focus on the
...
one. We show that this STF model implemented with a task-based runtime
system can lead to the implementation of a complex algorithm such as a
...
for solving SPD sparse systems. In particular we will focus on a
DAG-based variant of this algorithm that has been proven to be
extremely efficient on multicore architectures such as
in~\cite{h.r.s:10} with the \ma solver that will constitutes the basis
of this work.

The supernodal method is a factorization algorithm for sparse matrices
relying on an \textit{elimination tree} that expresses the
dependencies between the coefficients during the elimination of
unknowns. The elimination tree is computed during an analysis phase
preceding the factorization and in our case we obtain it by using the
software package HSL\_MC78. At each node of the tree we have a dense
matrix referred to as \textit{nodal} matrix or \textit{supernode} that
are associated with a set of unknowns. The supernode correspond to a
set of column in the matrix that have a similar non-zero pattern. The
factorization is performed by traversing the elimination tree in a
topological order and performing a dense Cholesky factorization on
supernodes. Regarding the update of supernodes with respect to other
nodal factorizations they are several strategies such as
\textit{right-looking} updates where all ancestors nodes are updated
wit respect to the current node, or, \textit{left-looking} updates
where nodes are updated just before being factorized.
...
The popularity of task-based algorithms pushed the OpenMP board to
...
The two parallel implementations of our task-based Cholesky solver
presented in this section are based on the same STF-based code
corresponding to the pseudo-code shown in
Figure~\ref{fig:spllt-facto-pseudocode}. In this code the sparse
Cholesky factorization is decomposed into six different type of tasks
associated to the following kernels that are based on the routines
found in \ma:
...
\item \texttt{init(snode)}: initializes the supernode \texttt{snode}
  such as copying the coefficient form the original matrix into the
  blocks.
...
\item \texttt{solve(bc\_kk, bc\_ik)}: perform the triangular solve of
  ...
  column.
\item \texttt{update(bc\_ik, bc\_jk, bc\_ij)}: perform the update
...
\item \texttt{update\_between(snode, bc\_ik, bc\_jk, anode, bc\_ij)}:
  perform the update operation of the block \texttt{bc\_ij} from the
  supernode \texttt{anode} with the blocks \texttt{bc\_ik} and
  \texttt{bc\_jk} from the descendent supernode \texttt{snode}.
\end{itemize}
...
numerical tasks are submitted in the same order as in sequential. In
order to guarantee the correctness of the algorithm, the runtime
ensure sequential consistency by using the data-access information
that we provide with the task submission. Note that the \texttt{alloc}
task is done sequentially because we need to allocate the data
structure of blocks to be able to submitted the task working on these
blocks. In the case of \openmp, blocks are identified using data
pointer and we associate these pointer with a data access when
submitting a task. Therefore we must allocate blocks before the being
able to submit tasks manipulating them. In the case of \starpu, blocks
are associated with a handle that is set up in this \texttt{alloc}
...
detecting when a data is written for the first time and allocate it
using the information contained in the handle.

The \texttt{init} task is responsible for setting up the block by in
a given supernode. In our code a supernode is represented by a
symbolic data structure denoted \texttt{snode} and we indicate that we
modify this structure by using the \textit{write} data-access in the
