...
model. The DAG is implicitly represented and the DAG is progressively
unrolled during the execution. However, the built-in scheduler,
implemented with OpenMP without using the tasking capabilities of the
recent version of the standard, is designed specifically for the
solver and rely on the knowledge of the algorithm.
...
\begin{table}[htbp]
    \begin{center}
      \texttt{ \input{data/cn255/table_cmp_task_insert.tex}}
    \end{center}
    \caption{\label{table:exp-times}Times (seconds) spend by the
      master thread to submit all the tasks to the runtime system.}
\end{table}
