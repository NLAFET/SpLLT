...
\begin{frame}{Task-based Sparse Cholesky factorization}
  ...
  \uncover<2->{Supernodes are \alert{partitioned} into square \db{blocks}
    (\texttt{nb x nb}) on which operations are applied.}\uncover<3->{
    The \alert{DAG} replaces the elimination tree for representing the
    dependencies.}
...
\begin{frame}{PTG: example}
  \begin{center}
    \lstinputlisting[width=0.1\textwidth]{listings/seq-example.c}%
